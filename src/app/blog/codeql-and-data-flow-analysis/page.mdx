...
export const article = {
  ...
  description:
    'An introduction to static analysis, taint tracking, and the problems that CodeQL is solving under the hood.',
}
...
In other words, find $\overline{X}=F(\overline{X})$ where $\overline{X}$ is a vector of "In's" and "Out's" representing the set of variables tained before and after
